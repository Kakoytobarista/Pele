...
CRONJOBS = [
    ('*/3 * * * *', 'core.cron.notification_job', '>> ' + os.path.join(BASE_DIR,
     'logs/notif_appointment.log' + ' 2>&1 ')),
    ('*/3 * * * *', 'core.cron.remove_unusable_appointment_job', '>> ' + os.path.join(BASE_DIR,
     'logs/del_appointment.log' + ' 2>&1 ')),
    ('10 8 * * *', 'core.cron.clean_log_file_job', '>> ' + os.path.join(BASE_DIR,
